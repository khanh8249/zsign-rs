...
/// Per-bundle profile map: bundle_id -> path to .mobileprovision file.
pub type ProfileMap = HashMap<String, PathBuf>;
...
pub struct IpaSigner<'a> {
    ...
    /// Per-bundle profiles map (bundle_id -> profile path).
    /// Dùng cho extension/framework khi sign nested bundles.
...
impl<'a> IpaSigner<'a> {
    ...
    /// PATCH: Set per-bundle profiles map (cho extension/framework).
    ///
    /// Key = bundle identifier (e.g. "com.example.app.extension")
    /// Value = path tới .mobileprovision file cho bundle đó.
